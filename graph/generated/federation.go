...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "FrameSide":
		resolverName, err := entityResolverNameForFrameSide(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "FrameSide": %w`, err)
		}
		switch resolverName {
	...
	case "Hive":
		resolverName, err := entityResolverNameForHive(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Hive": %w`, err)
		}
		switch resolverName {
...
func entityResolverNameForFrameSide(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for FrameSide", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for FrameSide", ErrTypeNotFound))
			break
		}
		return "findFrameSideByID", nil
	}
	return "", fmt.Errorf("%w for FrameSide due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForHive(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Hive", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Hive", ErrTypeNotFound))
			break
		}
		return "findHiveByID", nil
	}
	return "", fmt.Errorf("%w for Hive due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
