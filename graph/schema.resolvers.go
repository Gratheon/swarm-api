...
/*
	func (r *frameSideResolver) IsQueenConfirmed(ctx context.Context, obj *model.FrameSide) (bool, error) {
	// This field is directly mapped from the model struct by default.
	// This resolver is likely redundant unless specific logic is needed.
	return obj.IsQueenConfirmed, nil
}
func (r *frameSideResolver) FrameID(ctx context.Context, obj *model.FrameSide) (*string, error) {
	// obj is the FrameSide for which we need to find the parent Frame ID.
	if obj.ID == nil {
		logger.LogError(fmt.Errorf("cannot resolve FrameID for FrameSide with nil ID"))
		return nil, fmt.Errorf("internal error: FrameSide has nil ID")
	}

	uid := ctx.Value("userID").(string)
	frameSideID, err := strconv.Atoi(*obj.ID)
	if err != nil {
		logger.LogError(fmt.Errorf("invalid FrameSide ID format: %s", *obj.ID))
		return nil, fmt.Errorf("internal error: invalid FrameSide ID format")
	}

	// Instantiate a Frame model to use its methods
	frameModel := &model.Frame{
		Db:     r.Resolver.Db, // Use the resolver's DB connection
		UserID: uid,
	}

	// Call the method to find the parent Frame ID
	parentFrameID, err := frameModel.GetFrameBySideID(frameSideID)
	if err != nil && err != sql.ErrNoRows {
		// Log actual DB errors
		logger.LogError(fmt.Errorf("error fetching parent frame for FrameSide %d: %w", frameSideID, err))
		return nil, fmt.Errorf("internal error: could not fetch parent frame")
	}

	if parentFrameID == nil {
		// No parent frame found, return nil as the schema allows
		logger.LogError(fmt.Errorf("no parent Frame found for FrameSide ID: %d", frameSideID)) // Use LogError
		return nil, nil
	}

	// Convert the found Frame ID (int) to a string pointer
	frameIDStr := strconv.Itoa(*parentFrameID)
	return &frameIDStr, nil
}
